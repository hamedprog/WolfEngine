...
const std::list<std::string> w_capturer_factory::get_video_capture_device_list(const std::regex &p_publish_filter, bool p_use_null_codec)
{
    ...
    if (std::regex_match("videocap://", p_publish_filter) && !p_use_null_codec)
    {
        ...
        if (_info != nullptr)
        {
            ...
            for (auto i = 0; i < _num_video_devices; ++i)
            {
                std::array<char, W_MAX_PATH> _name = {0};
                std::array<char, W_MAX_PATH> _id = {0};

                if (_info->GetDeviceName(i, _name.data(), W_MAX_PATH, _id.data(), W_MAX_PATH) != -1)
                {
...
static rtc::scoped_refptr<webrtc::VideoTrackSourceInterface> create_video_source(const std::string &p_video_url,
...
static const std::list<std::string> get_audio_capture_device_list(const std::regex &p_publish_filter,
                                                                  rtc::scoped_refptr<webrtc::AudioDeviceModule> p_audio_device_module)
{
    ...
    {
        int16_t num_audioDevices = p_audio_device_module->RecordingDevices();
        for (auto i = 0; i < num_audioDevices; ++i)
        {
            std::array<char, W_MAX_PATH> _name = {0};
            std::array<char, W_MAX_PATH> _id = {0};
...
static rtc::scoped_refptr<webrtc::AudioSourceInterface> create_audio_source(const std::string &p_audio_url,
                                                                            const std::map<std::string, std::string> &p_opts,
                                                                            const std::regex &p_publish_filter,
                                                                            rtc::scoped_refptr<webrtc::PeerConnectionFactoryInterface> p_peer_connection_factory,
                                                                            ...
                                                                            rtc::scoped_refptr<webrtc::AudioDeviceModule> p_audio_device_module)
{
    ...
    else if (std::regex_match("audiocap://", p_publish_filter))
    {
        ...
        auto _num_audio_devices = p_audio_device_module->RecordingDevices();
        ...
        {
            for (int i = 0; i < _num_audio_devices; ++i)
            {
                if (p_audio_device_module->RecordingDeviceName(i, _name.data(), _id.data()) != -1)
                {
        ...
        if ((_idx_audio_device >= 0) && (_idx_audio_device < _num_audio_devices))
        {
            ...
            audioSource = p_peer_connection_factory->CreateAudioSource(opt);
        }
    }
    return audioSource;
}